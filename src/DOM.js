      ...
      for (let j = 0; j < player.grid[i].length; j++) {
        ...
        if (state === 'initial') {
          ...
          cell.addEventListener('click', (e) => {
            if (type !== 'computer' && e.isTrusted) return; //e.isTrusted returns true if the event was generated by a real mouse click and false if it was generated by click() fn
            if (!hasAttacked && player.attacks[i][j] === null) {
              ...
              if (player.attacks[i][j] === 1) {
                ...
                if (shipData[0].isSunk()) {
                  const [_, start, end, orientation] = shipData;

                  const startRow =
                    orientation === 'horizontal' ? i - 1 : start - 1;
                  const endRow = orientation === 'horizontal' ? i + 1 : end + 1;
                  const startCol =
                    orientation === 'horizontal' ? start - 1 : j - 1;
                  const endCol = orientation === 'horizontal' ? end + 1 : j + 1;

                  for (let r = startRow; r <= endRow; r++) {
                    for (let c = startCol; c <= endCol; c++) {
                      if (r >= 0 && r <= 9 && c >= 0 && c <= 9) {
                        if (player.attacks[r][c] === null) {
                          player.attacks[r][c] = 0;
                        }
                      }
                    }
                  }
